        ...
        for v in p:
            for v2 in v:
                for note in v2:
                    ...
                    if note.duration > 2:
                        note.env = f"[[0, 1, 0.8, 0.8, 0], [0.04, 0.1, {note.duration - 0.04 - 0.1 - 0.15}, 0.15], " \
                                        f"[4, -4, 0, -4]]"
                    elif note.duration > 1:
                        note.env = f"[[0, 1, 0.8, 0.8, 0], [0.03, 0.09, {note.duration - 0.03 - 0.09 - 0.13}, 0.13], " \
                                        f"[4, -4, 0, -4]]"
                    else:
                        note.env = f"[[0, 1, 0.8, 0.8, 0], [0.02, 0.08, {note.duration - 0.02 - 0.08 - 0.03}, 0.03], " \
                                        f"[4, -4, 0, -4]]"
